...
async function main() {
  console.log('=== Push WF13: Google Sheet Proxy ===\n');

  const wfJson = JSON.parse(fs.readFileSync(path.join(WF_DIR, 'wf13-gsheet-proxy.json'), 'utf-8'));
  ...
  if (res.status < 300) {
    ...
    console.log(`  Created WF13 with ID: ${newId}`);
    // Activate
    const actRes = await n8nAPI('POST', `/api/v1/workflows/${newId}/activate`);
    if (actRes.status < 300) console.log(`  Activated WF13`);
    ...
    console.log('  If WF13 already exists, update it manually or use its existing ID.');
  }
}
