...
const WF_ID = 'BkHh488ri25UHu8g';

console.log('=== Push wf-execution-stats ===\n');

const wf = JSON.parse(fs.readFileSync(path.join(WF_DIR, 'wf-execution-stats.json'), 'utf-8'));

console.log('Deactivating...');
await n8nAPI('POST', `/api/v1/workflows/${WF_ID}/deactivate`);

console.log('Updating...');
const res = await n8nAPI('PUT', `/api/v1/workflows/${WF_ID}`, stripForPush(wf));
if (res.status < 300) {
  console.log('  OK');
} else {
