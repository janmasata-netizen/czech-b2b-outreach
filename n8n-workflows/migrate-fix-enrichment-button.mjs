/**
 * Migration: Fix get_import_group_stats() — remove 'new' from in_progress_count
 *
 * 'new' means "waiting to be enriched" (pre-enrichment), not "in progress".
 * Including it locked the "Start Enrichment" button after full_pipeline imports.
 *
 * Run once: node migrate-fix-enrichment-button.mjs
 */
...
const steps = [
  ['Fix get_import_group_stats() — remove new from in_progress_count', `
    ...
    AS $$
      SELECT
        ...
        COUNT(l.id) FILTER (WHERE l.status IN ('enriching','enriched','email_discovery','email_verified','needs_review')) AS in_progress_count
...
console.log('Running enrichment button fix migration...\n');
let ok = 0, fail = 0;
