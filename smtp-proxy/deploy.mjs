#!/usr/bin/env node
/**
 * Deploy smtp-proxy to VPS via SSH/SFTP
...
const VPS_USER   = 'root';
const REMOTE_DIR = '/docker/smtp-proxy';
...
async function deploy() {
  console.log('=== Deploying smtp-proxy to VPS ===\n');
  ...
  try {
    const health = await execSSH(conn, `curl -s http://127.0.0.1:3002/health`);
    console.log('  Health check:', health);
  } catch (e) {
    console.warn('  Health check failed:', e.message);
    console.warn('  Container may still be starting — check manually: curl http://127.0.0.1:3002/health');
  }

  // Test n8n→proxy connectivity (Docker DNS)
  console.log('\nTesting n8n→proxy connectivity...');
  try {
    const n8nContainer = (await execSSH(conn, `docker ps --filter name=n8n --format "{{.Names}}" | head -1`)).trim();
    if (n8nContainer) {
      const proxyHealth = await execSSH(conn, `docker exec ${n8nContainer} wget -qO- http://smtp-proxy:3002/health 2>&1`);
      console.log(`  n8n (${n8nContainer}) → smtp-proxy: ${proxyHealth}`);
    } else {
      console.warn('  Could not find n8n container — test manually');
    }
  } catch (e) {
    console.warn('  n8n→proxy test failed:', e.message.slice(0, 100));
    console.warn('  Trying fallback via Docker bridge gateway...');
    try {
      const n8nContainer = (await execSSH(conn, `docker ps --filter name=n8n --format "{{.Names}}" | head -1`)).trim();
      if (n8nContainer) {
        const fallbackHealth = await execSSH(conn, `docker exec ${n8nContainer} wget -qO- http://172.17.0.1:3002/health 2>&1`);
        console.log(`  n8n via bridge: ${fallbackHealth}`);
      }
    } catch (e2) {
      console.warn('  Bridge fallback also failed:', e2.message.slice(0, 100));
    }
  }

  conn.end();
  console.log('\nDeploy complete! SMTP proxy at 127.0.0.1:3002 (VPS-local only)');
  console.log('n8n reaches it at http://smtp-proxy:3002 (Docker DNS via shared network)');
}
