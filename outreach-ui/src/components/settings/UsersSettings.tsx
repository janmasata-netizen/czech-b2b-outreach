...
import { useUsers, useCreateUser, useDeleteUser, useUpdateUserPassword, useUpdateOwnPassword, type AppUser } from '@/hooks/useUsers';
...
function CreateUserModal({ open, onClose, teamOptions }: {
  ...
  async function handleSave() {
    if (!form.email.trim() || !form.password.trim() || !form.full_name.trim()) {
      toast.error('Vyplňte jméno, e-mail a heslo'); return;
    }
    if (!form.team_id) { toast.error('Vyberte tým'); return; }
    try {
