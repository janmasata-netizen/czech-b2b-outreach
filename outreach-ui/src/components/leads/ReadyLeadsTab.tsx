...
import { useReadyLeads } from '@/hooks/useLeads';
...
export default function ReadyLeadsTab() {
  ...
  const { data: leads, isLoading } = useReadyLeads();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
          {t('leads.readyLeadsCount', { count: leads.length })}
        </div>
        {selected.length > 0 && (
          <GlassButton
            variant="primary"
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                ...
                {[t('leads.tableHeaders.company'), t('leads.tableHeaders.ico'), t('leads.tableHeaders.domain'), t('leads.tableHeaders.web'), 'Status', ''].map((h, i) => (
                  <th key={h || `action-${i}`} style={TH}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
